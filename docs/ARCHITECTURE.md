...
Inference workloads carry an explicit serving topology. Aggregated replicas have one TP/PP/EP/CP shape; P/D-disaggregated workloads carry independent prefill and decode shapes. The hardware envelope comes from the GPU-rack model carrying the most accelerators in the placed layout. Before replica-demand sizing, the workload engine derives the smallest attention-head-compatible TP whose per-GPU weight shard plus one stage-length KV sequence fits that platform's modeled usable HBM; PP/EP/CP stay user-controlled and request rate controls replica count. The memory result exposes both physical HBM and the runtime-usable fraction so a reserve cannot look like a different accelerator specification. It then derives stage GPU demand, TTFT/TPOT and KV-transfer cost from those shapes. Network > Traffic can select either a training or inference blueprint; the inference rate model combines that topology and P/D KV transfer with the GPU count, scale-up domain and NIC bandwidth of racks actually placed in the layout, then reports steady-state GB/s and utilization per fabric tier. Each report also carries the catalog platform, scale-up family, domain size, published bidirectional bandwidth, bus-bandwidth factor, and resulting effective one-way capacity, preventing an AMD Infinity Fabric envelope from being labelled or evaluated as NVLink.
