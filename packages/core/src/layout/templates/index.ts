...
export const LAYOUT_TEMPLATES: LayoutTemplate[] = [
  ...
  },
  {
    ...
    computeSlots: [primary(['rackscale-liquid-72', ...NVIDIA_RACK_SCALE], 'rackscale-liquid-72', ['rack-scale-liquid'], ACCEPTS_HPR_RACKSCALE)],
    defaults: STD_POLICY_V2HS,
    source: 'estimate',
  ...
  },
  {
    ...
    computeSlots: [primary(['rackscale-liquid-72-wide', ...WIDE_RACK_SCALE], 'rackscale-liquid-72-wide', ['rack-scale-liquid'], ACCEPTS_WIDE)],
    defaults: STD_POLICY_V2HS,
    source: 'estimate',
  ...
  // ── vendor samples ──
  {
    ...
    computeSlots: [primary([...NVIDIA_RACK_SCALE], 'nvidia-gb300-nvl72', ['rack-scale-liquid'], ACCEPTS_HPR_RACKSCALE)],
    defaults: POLICY,
    source: 'estimate',
  },
  {
    id: 'wide-rackscale-liquid-du',
    name: 'Wide-rack rack-scale liquid DU (2 x 8, HAC)',
    description: 'Two rows of 8 wide OU rack-scale systems (72-accelerator liquid-cooled domains) around a contained hot aisle, facility CDUs at the row start, network racks at the row end. Vendor sample on the wide rack form; row geometry is an AIDC Studio default (editable).',
    group: 'vendor-sample',
    profile: { rackForm: 'orw', rackPower: 'dc-busbar-50v-hpr' },
    pod: { ...REF_POD_TEMPLATE, gpuRackCatalogId: 'amd-helios-mi455x', racksPerRow: 8, rowsPerPod: 2, containment: 'hot-aisle', cduCatalogId: 'cdu-facility-2mw', scaleOutSwitchCatalogId: 'generic-roce-800', maxRacksPerRow: 16 },
    computeSlots: [primary([...WIDE_RACK_SCALE], 'amd-helios-mi455x', ['rack-scale-liquid'], ACCEPTS_WIDE)],
    defaults: POLICY,
    source: 'estimate',
    notes: 'Counterpart of the 21-inch rack-scale sample on the other rack form. The two are not interchangeable: this platform declares the wide rack form and the 21-inch sample takes orv3-hpr / orv3-mgx only, so neither template can list the other vendor. Both map to the same generic class (rackscale-liquid-72). Row counts, aisles and CDU positions are AIDC Studio estimates.',
  },
  {
    ...
    computeSlots: [primary([...NVIDIA_RACK_SCALE], 'nvidia-vr-nvl72', ['rack-scale-liquid'], ACCEPTS_HPR_RACKSCALE)],
    referenceUnit: {
  ...
  },
  {
    ...
    computeSlots: [primary(['nvidia-vr-nvl72', 'nvidia-gb300-nvl72'], 'nvidia-vr-nvl72', ['rack-scale-liquid'], ACCEPTS_HPR_RACKSCALE)],
    defaults: POLICY,
    source: 'estimate',
